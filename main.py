...
def main():
    # number of epochs to pretrain a model before making updates
    burnin_eps = [100]
    ...
    output_dir = homedir + f'/output/sss_UNET-REGULAR_{n_classes}-class'
    ...
    acc_f.write(f"acc_ch\tacc_tot\tburnin_ep\tperiod\tthresh_1\tthresh_2\n")

    for burnin_ep in burnin_eps:
        for thresh_1 in threshs_1:
            for thresh_2 in threshs_2:
                output_dir_i = output_dir + f'burnin-{burnin_ep}_period-{period}_thr_1-{thresh_1}_thr_2-{thresh_2}/'
                runtime, acc_ch, acc_total = train_wrapper(lr, epochs, output_dir_i, segy_filename, inp_res, train_files,
                                                           ...
                                                           use_crf, use_sisim, model_type, burnin_ep, period,
                                                           hood_kernels[:n_classes], window_size, overlap, loss_type,
                                                           mahal_thresh, weight_loss, weight_chan, rms_amp,
                                                           show_ood_maps, slice_n)

                runtime_f.write(f"{runtime} min\n")
                acc_f.write(f"{round(acc_ch, 2)}\t{round(acc_total, 2)}\t{burnin_ep}\t{period}\t{thresh_1}\t{thresh_2}\n")
