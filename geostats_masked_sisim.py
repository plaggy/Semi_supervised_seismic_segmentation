...
def ikrige(ix, iy, nx, ny, xx, yy, lktype, x, y, vr, sec, colocorr, gmean, lvm, close, covtab, nctx, ncty, icnode,
           ixnode, iynode, cnodev, cnodex, cnodey, nst, c0, PMX, cc, aa, it, ang, anis, rotmat, maxcov, MAXCTX, MAXCTY,
           MAXKR1, MAXKR2):
    """GSLIB's KRIGE subroutine (Deutsch and Journel, 1998) converted from the
                ...
                if na == 0:  # accounting for min index of 0 - one sample case na = 0
                    ...
                    for j in range(0, na):
                        ...
                        for i in range(0, na):  # was j - want full matrix
                            iin = iin + 1
                            a[iin] = cova2(xa[i], ya[i], xa[j], ya[j], nst, c0, PMX, cc, aa, it, ang, anis, rotmat,
                                           maxcov)
                        if ktype == 1:
                            iin = iin + 1
                            a[iin] = unbias
                        ...
                        if ndb <= 1:
                            ...
                            for j1 in range(0, ndb):
                                cb = cb + cova2(xx, yy, xdb[j1], ydb[j1], nst, c0, PMX, cc, aa, it, ang, anis, rotmat,
                                                maxcov)
                                dx = xx - xdb[j1]
                                dy = yy - ydb[j1]
                                if (dx * dx + dy * dy) < EPSLON:
            ...
            for ic in range(0, ncut):
                ...
                if nca == -1:
                    ...
                    for j in range(0, na):
                        ...
                        for i in range(0, na):  # was j - want full matrix
                            iin = iin + 1
                            a[iin] = cova2(xa[i], ya[i], xa[j], ya[j], nst[ic], c0[ic], PMX, cc[:, ic], aa[:, ic],
                                           it[:, ic], ang[:, ic], anis[:, ic], rotmat[ic], maxcov[ic])
                        if ktype == 1:
                            iin = iin + 1
                            a[iin] = maxcov[ic]
                        r[j] = cova2(xloc, yloc, xa[j], ya[j], nst[ic], c0[ic], PMX, cc[:, ic], aa[:, ic], it[:, ic],
...
def sisim(df, xcol, ycol, vcol, ivtype, koption, ncut, thresh, gcdf, trend, tmin, tmax, zmin, zmax, ltail, ltpar,
          middle, mpar, utail, utpar, nx, xmn, xsiz, ny, ymn, ysiz, seed, ndmin,
          ndmax, nodmax, mults, nmult, noct, radius, ktype, vario, mask_idc):
    ...
    cov_table, tmp2, order, ixnode, iynode, nlooku, nctx, ncty = ctable(MAXNOD, MAXCXY, MAXCTX, MAXCTY, MXY,
                                                                        xsiz, ysiz, isrot, nx, ny, nst[0], c0[0], cc[0],
                                                                        aa[0], it[0], ang[0], anis[0], global_rotmat,
                                                                        radsqd)
    ...
    sim = np.random.rand(nx * ny)
    '''
    order = np.zeros(len(mask_idc))
    ind = 0
    for ixy in range(len(mask_idc)):
        order[ixy] = ind  
        ind = ind + 1
...
#            print('multi grid - nnx, nny'); print(nnx,nny)
    ...
    np.random.shuffle(mask_idc)

    # As the data to the closest grid node:
    '''
    TINY = 0.0001
    for idd in range(len(mask_idc)):
#        print('data'); print(x[idd],y[idd])
        ix = mask_idc[idd][2] # getindex(nx,xmn,xsiz,x[idd])
        iy = mask_idc[idd][1] # getindex(ny,ymn,ysiz,y[idd])
        ind = ix + (iy-1)*nx
        xx  = xmn + (ix)*xsiz
        yy  = ymn + (iy)*ysiz
#        print('xx, yy' + str(xx) + ',' + str(yy))
...
# Another data values into the simulated grid:
    ...
    irepo = max(1, min((len(mask_idc) / 10), 10000))
    ...
    for ind in range(len(mask_idc)):
        ...
        index = mask_idc[ind][1] * nx + mask_idc[ind][2]
        if (sim[index] > (UNEST + EPSLON)) or (sim[index] < (UNEST * 2.0)): continue
        ix = mask_idc[ind][2]
        iy = mask_idc[ind][1]
        xx = xmn + (ix) * xsiz
        ...
        ncnode, icnode, cnodev, cnodex, cnodey = srchnd(ix, iy, nx, ny, xmn, ymn, xsiz, ysiz, sim, noct, nodmax, ixnode,
                                                        iynode, nlooku, nctx, ncty, UNEST)
        ...
        if nclose + ncnode <= 0:
            ...
            zval = beyond(ivtype, ncut, thresh, gcdf, ng, gcut, gcdf, zmin, zmax, ltail, ltpar, middle, mpar, utail,
                          utpar, zval, cdfval)
        else:
            #            print('kriging')
            ...
            for ic in range(0, ncut):
                ...
                ccdf[ic], cstdev = ikrige(ix, iy, nx, ny, xx, yy, ktype, x, y, vr[:, ic], sec, colocorr, gmean,
                                          trend[:, ic], nums, cov_table, nctx, ncty,
                                          icnode, ixnode, iynode, cnodeiv[ic], cnodex, cnodey, nst[ic], c0[ic], 9999.9,
                                          cc[ic], aa[ic], it[ic], ang[ic], anis[ic],
            ...
            zval = beyond(ivtype, ncut, thresh, ccdfo, ng, gcut, gcdf, zmin, zmax, ltail, ltpar, middle, mpar, utail,
                          utpar, zval, cdfval)
        sim[index] = zval
    #       print('zval'); print(zval)

    # END MAIN LOOP OVER SIMULATIONS:
    sim_out = np.ones((ny, nx)) * -1
    for ind in range(len(mask_idc)):
        iy = mask_idc[ind][1]
        ix = mask_idc[ind][2]
        sim_out[iy, ix] = sim[iy * nx + ix]
...
def semipartial_corr(
        C):  # Michael Pyrcz modified the function above by Fabian Pedregosa-Izquierdo, f@bianp.net for semipartial correlation
    C = np.asarray(C)
    p = C.shape[1]
    P_corr = np.zeros((p, p), dtype=np.float)
    for i in range(p):
